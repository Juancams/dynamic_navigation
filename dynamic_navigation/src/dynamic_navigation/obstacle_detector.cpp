  ...
  tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_))
{
  ...
  tfScanSub_ = std::make_shared<tf2_ros::MessageFilter<sensor_msgs::msg::LaserScan>>(
    scanSub_, *tf_buffer_, baseFrameId_, 100,
    node_->get_node_logging_interface(), node_->get_node_clock_interface(), buffer_timeout);
  ...
  if (tf_buffer_->canTransform("map", "base_footprint", tf2::TimePointZero)) {
    ...
    for (int i = 0; i < num_points; ++i) {
      ...
      if (r > min_lenght_ && !std::isnan(r)) {
        ...
        geometry_msgs::msg::TransformStamped scan2bf = tf_buffer_->lookupTransform(
        ...
        if (r != max_lenght_) {
          tf2::Transform map2obj_transform(tf2::Quaternion(
              map2bf.transform.rotation.x,
              map2bf.transform.rotation.y,
              map2bf.transform.rotation.z,
              map2bf.transform.rotation.w),
            tf2::Vector3(
              map2bf.transform.translation.x,
...
void ObstacleDetector::posCallback(
  const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr pos)
{
...
void ObstacleDetector::setDecrementCost1Q(
  unsigned int point_A_x, unsigned int point_A_y,
  unsigned int point_B_x, unsigned int point_B_y, int mod_x,
  int mod_y)
{
...
void ObstacleDetector::setDecrementCost2Q(
  unsigned int point_A_x, unsigned int point_A_y,
  unsigned int point_B_x, unsigned int point_B_y, int mod_x,
  int mod_y)
{
...
void ObstacleDetector::setDecrementCost3Q(
  unsigned int point_A_x, unsigned int point_A_y,
  unsigned int point_B_x, unsigned int point_B_y, int mod_x,
  int mod_y)
{
...
void ObstacleDetector::setDecrementCost4Q(
  unsigned int point_A_x, unsigned int point_A_y,
  unsigned int point_B_x, unsigned int point_B_y, int mod_x,
  int mod_y)
{
...
void ObstacleDetector::decrementCostProcedure()
{
  ...
  if (rclcpp::Clock().now() > last_exec_dec_ + rclcpp::Duration(std::chrono::seconds(1))) {
    for (auto & point : point_list_) {
      tf2::Transform map2bf_trans(tf2::Quaternion(
          map2bf.transform.rotation.x,
          map2bf.transform.rotation.y,
          map2bf.transform.rotation.z,
          map2bf.transform.rotation.w),
        tf2::Vector3(
          ...
          map2bf.transform.translation.y,
          map2bf.transform.translation.z));
      tf2::Transform point_temp(tf2::Quaternion(
